...
//! `project`/`song` settings reordered into a fixed canonical order, and at
//! most one blank line preserved anywhere the author left one. It refuses to
//! format source with lexical or syntax diagnostics, matching
//! `symphra-syntax`'s own guidance not to trust an AST produced alongside
//! diagnostics.
//!
