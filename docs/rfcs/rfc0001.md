...
Status: **implemented**. All ten proposals landed, one slice at a time, in
the order given under [Suggested implementation order](#suggested-implementation-order).
Where implementation changed a proposal, the section says so and why.

| Proposal | Status |
| --- | --- |
| S1 `* N` repetition | Implemented |
| S2 `velocity A..B` ramps | Implemented |
| S3 `[ … ]` subdivision | Implemented (with a corrected desugaring) |
| S4 `sequence step D` | Implemented |
| S5 chord symbols | Implemented |
| S6 pattern derivation | Implemented (a narrower stage list) |
| S7 `arpeggiate` | Implemented |
| S8 track overrides | Implemented |
| S9 `repeat fit` | Implemented |
| S10 effect presets | Implemented |
...
Halves round away from zero, so a descending ramp mirrors an ascending one.
Both endpoints are checked against the 0–127 MIDI range.
...
```text
Subdivision = "[" StepItem { StepItem } "]"
```

`choose` may not appear inside a subdivision, for the same reason it may not
be repeated.
...
A step resolution may also be written in bars (`steps 1bar`), which is what
makes a bar-sized cell worth subdividing. That is part of this proposal.

**Desugaring — corrected during implementation.** This section originally
proposed flattening the body onto the least common multiple of all
subdivision counts, so that `steps 1bar` with cells of 2/4/8/16 became the
64-cell `steps 1/16` pattern the example writes by hand. That was
implemented differently, because LCM flattening makes an event's duration
depend on *unrelated cells*: adding a `[ … * 16 ]` elsewhere in the body
would silently shorten the claps in the first bar from half a bar to a
sixteenth. A construct whose meaning changes at a distance is not sugar.

What is implemented instead is the local rule the syntax already suggests:
each item in a subdivision lasts `cell / count`, recursively. `[ cp cp ]` in
a one-bar cell is two claps of half a bar each. Steps already carry their own
duration — a `sequence` body has had per-item durations all along — so this
needs no new representation.

The visible consequence: because `symphra-render` truncates a sample at the
end of its step, subdivided drum cells ring longer than the equivalent
hand-written `1/16` grid with rests. Both spellings stay available, and
`001-example.sym` keeps its `1/16` grid for that reason.

Guard: the whole body's event count — through repetitions and into
subdivisions — is counted before anything is built, and rejected past
`MAX_EXPANDED_ITEMS`. A subdivision that cannot be represented as a
duration (past a `u32` denominator) is reported at the subdivision.
...
Note this also makes `rest` (with no `for`) legal inside a `step`-carrying
sequence.

The "41 lines → 6" this section originally claimed assumed several items per
line. `symphra-fmt` prints one sequence item per line, so what `step`
actually removes is the repeated `for 1/8` on every line, not the lines
themselves. Packing a `step`-carrying sequence onto filled lines would be a
formatter change, and is left as an open question below.
...
PatternStage = "transpose" … | "repeat" … | "reverse"
```

The implemented stage list is narrower than this section first proposed.
`gain` is out because it scales amplitude, and a pattern has no amplitude —
its analogue, velocity, is a different quantity, and giving one keyword two
meanings would be worse than leaving it out. `gate` and `trigger_with` are
out until their pattern-level meaning is pinned down: both are defined today
against a performance's slots rather than a pattern's steps. The
track-domain stages (`at`, `pan`, `speed`, `choose_sample`) were never
candidates.

Cycles are impossible by construction rather than by check: a derivation
resolves only against patterns declared above it.
...
| --- | --- |
| `as_written` | `0, 1, 2, …`, over the chord's pitches in written order |
...
`as_written` earns its name by being the one style that does *not* sort the
chord's tones ascending first — it walks them in the order the chord wrote
them.
...
The derived-declaration alternative this section also sketched
(`track x = base { ... }`) was **not** implemented: with per-reference
overrides in place it earns its keep only when the same override repeats
across sections, which the example never does. It stays available as a
follow-up.

Two notes from the implementation:

- An override synthesizes a per-section copy of the track, with its own
  identity. `chance` rolls are seeded from that identity, so an overridden
  reference rolls like the separate declaration it replaces — the same as
  today, where a track played in two sections already rolls differently in
  each.
- A `volume` override scales the declaration's gain rather than replacing
  the computed value, so the play pipeline's own `gain` stage still applies.
  The scaling costs one float ULP against writing the same volume out by
  hand.
...
A pattern whose length depends on a `choose` roll is rejected too, and
`repeat fit` is rejected on a pattern derivation (S6), which has no section
to fill.
...
5. ~~**`octaves` default.**~~ Settled while implementing S7: the default is
   unbounded, so `up_down` reaches the root an octave up the way the
   example's hand-written arpeggio does. `octaves N` opts into wrapping.
6. ~~**Should `[ … ]` also work in `sequence` bodies?**~~ Settled while
   implementing S3: sequences carry explicit durations already, so
   subdivision has nothing to divide there. `steps` only.
7. **Should the formatter pack `step` sequences onto filled lines?** One
   item per line is the rule everywhere else, but a `step`-carrying
   sequence is exactly the case where a bar per line would read better.
8. ~~**`rhythm` bodies and `[ … ]`.**~~ Settled while implementing S1:
...
Every proposal in this RFC is implemented, and
`examples/draft-0.1/001-example.sym` uses them: **664 lines → 504**, with the
rendered audio unchanged at every step (each slice was checked by compiling
the file before and after and comparing the lowered HIR, or the scheduled
score where node ids necessarily shift).

Where the 160 lines went:

| Region | Before | After | Sugar |
| --- | --- | --- | --- |
| `arpeggio` | 37 | 4 | S7 |
| `snare_roll` | 74 | 14 | S1, S2 |
| Drum `steps` patterns | 53 | 20 | S1 |
| `drop_chords` | 6 | 1 | S6 |
| Duplicate tracks | 50 | 0 | S8, S9 |
| `effect reverb` blocks | 44 | 28 + 11 one-line references | S10 |
| `pad_chords` | 6 | 6 | S5 — same length, but it now says "Gmaj7" |
| `lead_line`, `bass_roots`, rhythms | — | — | S1, S4 — shorter lines, same count |

Two things did **not** shrink the way this RFC first estimated:

- **`sequence step D` (S4)** trims the repeated `for 1/8` from each line but
  not the lines themselves, because the formatter prints one sequence item
  per line. See open question 7.
- **`[ … ]` subdivision (S3)** is implemented, but the example keeps its
  `1/16` grid for `snare_roll`: with local subdivision semantics the claps
  would ring for half a bar instead of a sixteenth (see S3 above), so the
  compact spelling is a different sound rather than the same one.

## Follow-ups

Left deliberately undone, in rough order of appeal:

1. `track x = base { ... }` derived declarations (S8), for an override
   repeated across several sections.
2. `gate` and `trigger_with` as pattern-derivation stages (S6), once their
   pattern-level meaning is pinned down.
3. Ramps on `gain`, `speed`, and `cutoff` (S2's open question 3).
4. Formatter line-filling for `step`-carrying sequences (open question 7).
