...
song "S" {
  ...
  track chords role harmony {
    instrument lead
    volume -6 db
    play harmony |> trigger_with stabs |> gate 80% |> transpose +2 st |> gain 0.8 |> repeat 2 |> reverse |> chance 15% { transpose +12st } |> pan alternate(30%,70%)
  }
}
"#;
...
song "S" {
  ...
  track chords role harmony {
    instrument lead
    volume -6 db
    play harmony |> trigger_with stabs |> gate 80% |> transpose 2 st |> gain 0.8 |> repeat 2 |> reverse |> chance 15% { transpose 12 st } |> pan alternate(30%, 70%)
  }
}
"#;
